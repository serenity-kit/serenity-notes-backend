...
const deleteRepositoryMutationString = `
...
it("delete repository fails since it must be the author", async () => {
  const request = async () =>
    await user2ClientA.request(deleteRepositoryMutationString, {
      input: {
        repositoryId: createRepositoryMutation.createRepository.repository.id,
      },
    });
...
it("delete repository succeeds", async () => {
  const deleteRepositoryMutationResult: any = await clientA.request(
    deleteRepositoryMutationString,
    {
      input: {
        repositoryId: createRepositoryMutation.createRepository.repository.id,
      },
  ...
  expect(deleteRepositoryMutationResult.deleteRepository.success).toBe(true);
});
