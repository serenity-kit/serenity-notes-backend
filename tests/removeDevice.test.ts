...
it("fails to delete device B from unauthenticated client", async () => {
  const client = new GraphQLClient("http://localhost:4000/graphql");

  const request = async () =>
    await client.request(deleteDeviceString, {
...
it("delete device B from clientA", async () => {
  const deleteDeviceMutation: any = await clientA.request(
    `
      mutation deleteDevice($input: DeleteDeviceInput!) {
        deleteDevice(input: $input) {
          success
...
it("fails delete the user's last device", async () => {
  const request = async () =>
    await clientA.request(deleteDeviceString, {
      input: { deviceIdKey: deviceAKeys.idKey },
    });
