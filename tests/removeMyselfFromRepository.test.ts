...
it("remove myself as collaborator fails since it can't be the author", async () => {
  const request = async () =>
    await clientA.request(removeCollaboratorFromRepositoryMutationString, {
      input: {
        repositoryId: createRepositoryMutation.createRepository.repository.id,
...
it("remove myself as collaborator from repository succeeds", async () => {
  const removeCollaboratorFromRepositoryMutationResult: any = await user2ClientA.request(
