...
let groupSessionB: GroupSession;
let user1DeviceAGroupSessionMessageIds: string[] = [];
let user1DeviceBGroupSessionMessageIds: string[] = [];
let user1DeviceBRepositoryDevices: { idKey: string }[] = [];
let repositoryId = "";
...
const repositoryDevicesQueryString = `
...
const updateRepositoryContentAndGroupSessionMutationString = `
...
it("create a repository", async () => {
  ...
  const createRepositoryMutation: any = await clientA.request(
    `
  ...
  user1DeviceAGroupSessionMessageIds =
...
it("deviceA: fetch repositoryDevices and groupSessionMessageIdsMatchTargetDevices should be true", async () => {
  const repositoryDevicesQuery: any = await clientA.request(
    repositoryDevicesQueryString,
    {
      repositoryId: repositoryId,
      groupSessionMessageIds: user1DeviceAGroupSessionMessageIds,
    }
  );

  expect(
    repositoryDevicesQuery.repositoryDevices
      .groupSessionMessageIdsMatchTargetDevices
  ).toBe(true);
});
...
it("deviceA: fetch repositoryDevices and groupSessionMessageIdsMatchTargetDevices should be false", async () => {
  const repositoryDevicesQuery: any = await clientA.request(
    repositoryDevicesQueryString,
    {
      repositoryId: repositoryId,
...
it("update the repository from device A for device A & B", async () => {
  ...
  const updateRepositoryContentAndGroupSessionMutation: any = await clientA.request(
    updateRepositoryContentAndGroupSessionMutationString,
    {
      input: {
        repositoryId: repositoryId,
  ...
  user1DeviceAGroupSessionMessageIds =
    updateRepositoryContentAndGroupSessionMutation
      .updateRepositoryContentAndGroupSession.groupSessionMessageIds;
});
...
it("deviceB: fetch repositoryDevices and groupSessionMessageIdsMatchTargetDevices should be false", async () => {
  const repositoryDevicesQuery: any = await clientA.request(
    repositoryDevicesQueryString,
    {
      repositoryId: repositoryId,
      groupSessionMessageIds: user1DeviceBGroupSessionMessageIds,
    }
  );
  ...
  user1DeviceBRepositoryDevices =
    repositoryDevicesQuery.repositoryDevices.devices;
});

it("update the repository from device B for device A & B", async () => {
  groupSessionB = createGroupSession();
  const groupSessionMessages = await claimOneTimeKeysAndCreateGroupSessionMessages(
    clientB,
    deviceB,
    user1DeviceBRepositoryDevices,
  ...
  user1DeviceBGroupSessionMessageIds =
    updateRepositoryContentAndGroupSessionMutation
      .updateRepositoryContentAndGroupSession.groupSessionMessageIds;
});
...
it("fetch devices for the other user, verify them and add to repo (user1 to user2)", async () => {
  ...
  user1DeviceAGroupSessionMessageIds =
    addCollaboratorToRepositoriesMutationResult.addCollaboratorToRepositories
      .entries[0].groupSessionMessageIds;
});
...
it("deviceB: fetch repositoryDevices and groupSessionMessageIdsMatchTargetDevices should be true", async () => {
  const repositoryDevicesQuery: any = await clientA.request(
    repositoryDevicesQueryString,
    {
      repositoryId: repositoryId,
